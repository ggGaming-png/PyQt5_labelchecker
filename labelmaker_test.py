...
class picture(QWidget):
    def __init__(self, classlist):
        ...
        self.check_box = 0
        # 导入文件夹
        self.pic_path = "test_pic"
        self.res_path = "test_json"
        self.pic_path_list = []
        ...
        self.btn = QPushButton('结果路径设置', self)
        self.btn.setGeometry(QRect(120,10,110,35))
        self.btn.clicked.connect(self.setResPath)
    ...
    def opendir(self):
        self.pic_path = QFileDialog.getExistingDirectory(
            self, "请选择模板保存路径...", "./")
        print(self.pic_path)
    ...
    def writelabel(self, label, btn_list, id):
        ...
        now_txt_path = now_txt_path.replace(self.pic_path, self.res_path)
        print(now_txt_path)
        with open(now_txt_path, 'rb') as f:
            json_data = json.load(f)
            #print(json_data)
            json_data[index] = int(value)
            self.labelstr = "occasion: " + str(json_data.get("occasion",0)) + "  quality: " + str(json_data.get("quality",0)) + "  box: " + str(json_data.get("box",0))
            self.lb1.setText(self.labelstr)
            print(json_data)
            f.close()
    ...
    def showPicInfo(self):
        ...
        with open(now_txt_path, 'rb') as f:
            ...
            keys = json_data.keys()
            #防止出现有空缺的情况
            if "occasion" not in keys:
                json_data["occasion"] = 0
                self.check_occ +=1
    ...
    def setResPath(self):
        self.res_path = QFileDialog.getExistingDirectory(
            self, "请选择模板保存路径...", "./")
        print(self.res_path)
